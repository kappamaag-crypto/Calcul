...
## SYNC CHECKPOINT — 2026-09-23 — actual fwmark jump rule located
- [RESULT] Live `inet zapret2` table contains `postnat_hook` with `meta mark & 0x40000000 == 0x00000000`, `oifname @wanif`, `ip daddr != @nozapret`, then `jump postnat` (handle 195).
- [RESULT] The active `postnat` chain contains the three `50-wg4all` qnum `65300` WireGuard packet matches (handles 202-204 in this current ruleset) and the standard qnum `300` TCP/UDP rules.
- [IMPORTANT] Therefore the external fwmark hypothesis points to a real rule, but its causality is not established: we still do not know whether packets generated by `proton-test` enter `postnat_hook` with bit `0x40000000` already set.
- [SAFETY] Read-only inspection only; no nftables or routing state changed.
- [STATUS] `50-wg4all` functional integration = DONE; Proton/WireGuard validation = BLOCKED at missing peer response.

## SYNC CHECKPOINT — 2026-09-23 — WireGuard fwmark is off
- [RESULT] `wg show proton-test fwmark` returned `off`.
...
## SYNC CHECKPOINT — 2026-09-23 — OpenVPN crypto directives inspected
- [RESULT] `proton-nl-free-130-tcp.ovpn` explicitly contains `remote-cert-tls server` and `auth-user-pass`, but no explicit `auth`, `data-ciphers`, `data-ciphers-fallback`, or TLS version/cipher directives.
- [CONCLUSION] There is no obvious static crypto-parameter conflict visible in the profile; OpenVPN 2.7.6 is using defaults for unspecified directives.
...
- [RESULT 2026-09-23] Live `fakedsplit:pos=2` A/B test completed. OpenVPN reached `TCP connection established` to `84.20.27.33:443`, then `Server poll timeout` after ~10 s; no initialization/auth success.
