...
### 2026-09-22 — начало контролируемой router-side проверки кандидатов
- Пользователь подтвердил рабочий алгоритм: сохранить текущий NFQWS2_OPT → выбрать TCP-кандидата из 75 common → отдельно QUIC-1 → временный тест → TLS1.2/TLS1.3/QUIC → постоянная запись только при улучшении → YouTube/Telegram/WhatsApp.
- На роутере выполнена резервная копия: `/opt/zapret2/config.backup-before-router-selection`.
...
- Команда создания временной копии `/tmp/zapret2-config-test` выполнена без вывода, что соответствует успешному `cp`.
- Постоянный `/opt/zapret2/config` по-прежнему не изменялся.
