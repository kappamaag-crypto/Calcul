...
- [OBSERVED 2026-09-20] `nft list chain inet zapret2 prenat` also shows an empty `prenat` chain. Both principal NFQWS2 NAT-path chains (`postnat` and `prenat`) currently contain no queue rules. Service status remains `running`, so activation is incomplete/not functionally verified.
- [OBSERVED 2026-09-20] `ps w | grep '[n]fqws2'` confirms a live NFQWS2 process (PID 4375, user `daemon`) with `/opt/zapret2/nfq2/nfqws2`, including `--fwmark=0x40000000` and Zapret2 Lua init. The displayed `ps w` line is truncated, so the full argument list is not yet established. This confirms daemon presence only; `prenat`/`postnat` remain empty.
- [OBSERVED 2026-09-20] Full `/proc/4375/cmdline` confirms NFQWS2 is running with `--qnum=300` and three active protocol profiles: TCP/80 HTTP, TCP/443 TLS, UDP/443 QUIC. It uses the configured Lua init files and `--fwmark=0x40000000`. This closes daemon-argument verification; it does not establish that nftables queue rules are attached.
- [OBSERVED 2026-09-20] `nft -a list table inet zapret2 | grep -E 'queue|hook|qnum'` shows only hook-chain declarations; no `queue`/`qnum` rules are present anywhere in `inet zapret2`. Therefore the queue-expression module is loaded and NFQWS2 is running, but the nftables-to-NFQUEUE attachment is absent. Activation remains incomplete.
- [OBSERVED 2026-09-20] `/etc/init.d/zapret2` grep shows only NFQWS2 daemon definition/options and `run_daemon`; it contains no `queue`, `postnat`, `prenat`, or nft-rule insertion logic. Therefore the missing NFQUEUE rules are likely generated by a sourced Zapret2 helper/config script rather than this init script itself. No system change was made.
- [OBSERVED 2026-09-20] Zapret2 source search located the actual nftables queue insertion logic in `/opt/zapret2/common/nft.sh`. Functions around lines 469–581 emit `Inserting nftables...` and call `nft_insert_rule ... queue num ...`; `installer.sh` also explicitly lists `kmod-nft-queue` for nftables mode. This confirms the queue rules are generated by `common/nft.sh`, not `/etc/init.d/zapret2`. No source files were modified.

- [OBSERVED 2026-09-20] Read-only inspection of `/opt/zapret2/common/nft.sh` lines 450–590 confirms the NFQWS2 nftables path explicitly calls `nft_insert_rule` with `queue num $port bypass` for both postrouting and prerouting, IPv4 and IPv6. `get_postchain()` selects `postnat` when POSTNAT is enabled, otherwise `postrouting`; `get_prechain()` similarly selects `prenat` or `prerouting`. With current `DISABLE_IPV6=1`, the IPv6 branches are disabled.
- [CONFIRMED] The source logic passes the configured queue number into the queue expression; this matches the live NFQWS2 command line `--qnum=300`.
- [IMPORTANT] The inspected section does not itself show the implementation of `nft_insert_rule`; therefore the exact reason the queue rule insertion failed is not yet proven from this excerpt alone. The next read-only step is to locate and inspect the definition of `nft_insert_rule` and its command construction.
...
- [OBSERVED 2026-09-20] Local DoH listeners are present: https-dns-proxy PID 5011 listens TCP+UDP 127.0.0.1:5053; PID 3222 listens TCP+UDP 127.0.0.1:5054. Therefore the DNS outage is not caused by missing listeners. Combined with dnsmasq query saturation, the next question is whether the DoH proxy instances are successfully resolving upstream or are themselves stalled/failing.
- [USER REQUEST] User explicitly asked for concrete resolution rather than only further diagnosis. Resolution path: verify DoH proxy runtime/error state first; then, based on result, either restore working upstream DNS/DoH or temporarily bypass the DoH proxy to restore Internet, followed by controlled reintroduction. No change has yet been made.

- [ROOT CAUSE CONFIRMED 2026-09-20] Kernel log shows `https-dns-proxy` invoked the OOM killer at 21:27:07; process PID 3221 (`https-dns-proxy`) was killed by the kernel. This directly explains the DNS saturation (`Maximum number of concurrent DNS queries reached (max: 150)`) and the client DNS timeout. The issue is resource exhaustion in the current DoH proxy path, not absence of DNS listeners or basic fw4 forwarding.
- [RECOVERY PLAN] Restore client DNS first by bypassing the failing local DoH proxy path and using the known-working upstream DNS path via TP-Link (192.168.0.1), while preserving the DoH configuration for later controlled reintroduction. Do not touch Zapret2 during this recovery.
- [STATUS] DNS/DoH fault is confirmed; client Internet functional validation remains FAILED until DNS is restored.
...
SYNC 2026-09-21: Client HTTPS validation after DNS recovery remains PARTIAL/FAILED. User tested `https://example.com` and `https://www.google.com` from a MikroTik Wi-Fi client; both did not open, while YouTube opened. [CONFIRMED] Client Wi-Fi connectivity, DNS resolution, and at least some forwarded HTTPS traffic are functional; the failure is selective rather than total. [STATUS] STAGE 11 remains IN_PROGRESS; functional client Internet validation remains FAILED. [IMPORTANT] Do not infer that Zapret2 is fully working from YouTube alone. The next isolation step is to test HTTPS from the router itself, without changing Zapret2 or firewall state.

SYNC 2026-09-21: Router HTTPS test command was attempted with `wget -S`, but this BusyBox wget does not support option `-S`; it returned usage text and made no network request. No configuration or service state changed. [IMPORTANT] Do not treat this as an HTTPS connectivity result. [NEXT] Repeat the router-side HTTPS test using only supported BusyBox wget options: `wget -O /dev/null -T 10 https://example.com`.

SYNC 2026-09-21: Router-side HTTPS validation FAILED. Command: `wget -O /dev/null -T 10 https://example.com`. Result: DNS resolved example.com to 104.20.23.154, TCP connection to 104.20.23.154:443 timed out. [CONFIRMED] The HTTPS failure is not limited to Wi-Fi clients; the router itself cannot establish this direct HTTPS connection. [IMPORTANT] This makes client forwarding/NAT alone insufficient to explain the failure. Current evidence points toward the active Zapret2/NFQUEUE path or another router-side packet-processing interaction affecting TCP/443. Do not change strategy yet without inspecting the active NFQWS2 configuration and live counters. [STATUS] STAGE 11 remains IN_PROGRESS; functional Internet validation remains FAILED.

SYNC 2026-09-21: Active Zapret2 configuration inspected. Command: `grep -E '^(NFQWS2_|MODE_FILTER|FLOWOFFLOAD|INIT_APPLY_FW|DISABLE_IPV6)' /opt/zapret2/config`. [CONFIRMED] NFQWS2 is enabled; TCP ports 80,443 and UDP 443 are queued; packet limits are TCP OUT=20/IN=10 and UDP OUT=5/IN=3; MODE_FILTER=none; FLOWOFFLOAD=donttouch; INIT_APPLY_FW=1; DISABLE_IPV6=1. [IMPORTANT] The displayed `NFQWS2_OPT="` has no visible option value before the next variable, so the current strategy/options appear empty and must be verified from the surrounding config syntax before any strategy change. Do not modify Zapret2 yet.

SYNC 2026-09-21: Active Zapret2 configuration inspection. Command: `grep -E '^(NFQWS2_|MODE_FILTER|FLOWOFFLOAD|INIT_APPLY_FW|DISABLE_IPV6)' /opt/zapret2/config`. Result confirms `NFQWS2_ENABLE=1`, TCP ports 80,443, UDP 443, packet limits OUT=20/IN=10 for TCP and OUT=5/IN=3 for UDP, `MODE_FILTER=none`, `FLOWOFFLOAD=donttouch`, `INIT_APPLY_FW=1`, `DISABLE_IPV6=1`. The displayed `NFQWS2_OPT="` appears to have an unterminated opening quote in the visible output; this must be verified before any strategy change. [IMPORTANT] Do not modify the strategy yet. [NEXT] Perform one read-only inspection of the exact NFQWS2_OPT/config lines to determine whether the quote is genuinely unterminated or only a display/wrapping artifact.

SYNC 2026-09-21: Exact NFQWS2_OPT block inspected. The quote is properly closed; the previous apparent unterminated quote was a terminal line-wrap artifact. Current strategy text contains TCP/80 HTTP desync, TCP/443 TLS desync, and UDP/443 QUIC desync rules, each using `<HOSTLIST>` or `<HOSTLIST_NOAUTO>`; `MODE_FILTER=none` follows. [CONFIRMED] There is no syntax defect in the visible NFQWS2_OPT block. [IMPORTANT] Because MODE_FILTER=none is active while the strategy references hostlist placeholders, the effective runtime arguments should be inspected before changing strategy. [NEXT] Perform one read-only inspection of the running NFQWS2 process command line to see the actual expanded NFQWS2 arguments.

SYNC 2026-09-21: Running NFQWS2 command line inspected. Command: `tr '\0' ' ' < /proc/$(pidof nfqws2)/cmdline`. [CONFIRMED] NFQWS2 is running with qnum=300 and the expected HTTP/TLS/QUIC desync rules are fully expanded at runtime. Crucially, the running command line contains no `<HOSTLIST>` or `<HOSTLIST_NOAUTO>` filters; those placeholders from the config are absent from the effective runtime command. Therefore `MODE_FILTER=none` results in these desync rules applying to matching protocol traffic without hostlist restriction. [IMPORTANT] This is a strong candidate explanation for normal HTTPS traffic being disrupted: TCP/443 desync is being applied broadly, not selectively to a target hostlist. Do not change config yet. [NEXT] One read-only test should determine whether the timeout is caused by NFQWS2 by testing the same HTTPS endpoint with NFQUEUE bypassed/temporarily excluded only if a safe, reversible diagnostic mechanism is already available; otherwise first inspect the active nft queue rules and counters to establish exactly which chains/ports are feeding queue 300.

SYNC 2026-09-21: Live Zapret2 postnat chain inspected. Command: `nft -a list chain inet zapret2 postnat`. [CONFIRMED] IPv4 UDP/443 packets 1-5 and TCP/80,443 packets 1-20 are marked and sent to NFQUEUE 300 with `queue flags bypass`. TCP/443 is therefore explicitly intercepted by NFQWS2 for the first 20 original packets of each connection; UDP/443 for the first 5. [IMPORTANT] This rule is broad and is not hostlist-restricted. Combined with the router-side HTTPS timeout, the active TCP/443 NFQUEUE path remains a strong fault domain, but the rule itself does not prove NFQWS2 is the cause because `bypass` permits packets when no queue listener is available. [NEXT] Perform one read-only inspection of the full `inet zapret2` table to confirm the hook/chain structure and whether any additional rules participate in the path before considering a reversible strategy test.

SYNC 2026-09-21: Full live `inet zapret2` table inspected. [CONFIRMED] Zapret2 has postrouting and prerouting hooks that broadly feed IPv4 TCP/80,443 and UDP/443 traffic to NFQUEUE 300; replies are also queued for the first packets. `postnat_hook` applies to WAN output unless destination is in `@nozapret`; `prenat_hook` applies to WAN input unless source is in `@nozapret`. `forward_hook` and flow-offload chains are empty, so the active interception is specifically in postrouting/prerouting hooks. [CONFIRMED] `wanif` is `phy0-sta0`, `lanif` is `br-lan`, and the queue rules are not hostlist-restricted at nft level. [IMPORTANT] This makes the broad NFQUEUE interception a strong candidate for the router-side TCP/443 timeout, but a causal conclusion still requires a controlled bypass test. Before stopping/killing NFQWS2, inspect its parent process so we know whether a supervisor will immediately restart it.

SYNC 2026-09-21: Attempted parent-process inspection with `ps -o pid,ppid,args -p $(pidof nfqws2)`, but BusyBox 1.37.0 `ps` does not support `-o`; it returned usage text and made no state change. [IMPORTANT] No process/supervisor conclusion can be drawn from this attempt. [NEXT] Use the BusyBox-supported wide process listing in a read-only command and filter for nfqws2/zapret2.

SYNC 2026-09-21: BusyBox-supported process listing confirms NFQWS2 PID 4894 is running as user `daemon`, but `ps w` output does not expose PPID in this build. No state change occurred. [CONFIRMED] NFQWS2 remains active with the expected /opt/zapret2/nfq2/nfqws2 command. [NEXT] Read the kernel process-stat record for PID 4894 to obtain its PPID without stopping or restarting anything.

SYNC 2026-09-21: Controlled Zapret2 bypass test initiated. User executed `/etc/init.d/zapret2 stop`; output: "Clearing nftables". [CONFIRMED] Zapret2 service stop command completed without reported error and its nftables rules were cleared. [STATUS] NFQWS2/Zapret2 is intentionally stopped for the reversible HTTPS isolation test. No Zapret2 configuration files, USB storage, firewall UCI configuration, or DNS configuration were changed. [NEXT] Perform exactly one router-side HTTPS test to the same endpoint used before: `wget -O /dev/null -T 10 https://example.com`.

SYNC 2026-09-21: Controlled Zapret2 bypass test PASS. With Zapret2 stopped and its nftables rules cleared, the exact router-side HTTPS test `wget -O /dev/null -T 10 https://example.com` completed successfully: connection to 8.6.112.0:443 succeeded and 559 bytes were downloaded to /dev/null. [CONFIRMED] The previous TCP/443 HTTPS timeout was caused by the active Zapret2/NFQWS2 interception path, not by basic upstream connectivity, DNS, or the base firewall/NAT path. [IMPORTANT] This is a causal isolation result; do not treat the current broad NFQWS2 strategy as suitable in its present form. [STATUS] Zapret2 remains intentionally stopped after the bypass test. [NEXT] Restore Zapret2 only when needed, then redesign its filtering to avoid broad interception; do not re-enable the current broad rules blindly.

SYNC 2026-09-21: Next Zapret2 strategy step selected without additional connectivity tests. Official zapret2 config documentation confirms that `MODE_FILTER=hostlist` makes `<HOSTLIST>` expand to configured hostlists, while `MODE_FILTER=none` removes the hostlist markers and therefore leaves the strategy broad. citeturn0search0turn0search1 [DECISION] The current broad TCP/443 strategy will not be re-enabled. [NEXT] Change only `MODE_FILTER` from `none` to `hostlist`; do not change NFQWS2 strategy text, ports, packet limits, DNS, firewall, or storage. Zapret2 remains stopped until the configuration change is complete.

SYNC 2026-09-21: User executed the planned configuration-only command `sed -i 's/^MODE_FILTER=none$/MODE_FILTER=hostlist/' /opt/zapret2/config`. Terminal produced no output. [CONFIRMED] The command completed without reported error; no output is expected from successful sed editing. [STATUS] Zapret2 remains stopped; only MODE_FILTER was targeted. [NEXT] Start Zapret2 with the new hostlist mode; no additional read-only verification requested.

SYNC 2026-09-21: User started Zapret2 after changing `MODE_FILTER` to `hostlist`. [CONFIRMED] `/etc/init.d/zapret2 start` completed without reported error; NFQWS2 started on queue 300 and nftables rules were applied for TCP/80,443 and UDP/443. The expanded command line shows blank hostlist fields (double spaces after the HTTP/TLS filter declarations), so no hostlist names are visibly inserted into the runtime command. [STATUS] Zapret2 is running with hostlist mode. [NEXT] Perform the single functional HTTPS test already used for causal comparison: `wget -O /dev/null -T 10 https://example.com`. No additional diagnostic tests.

SYNC 2026-09-21: Hostlist-mode HTTPS control test FAILED. With `MODE_FILTER=hostlist`, the same router-side `wget -O /dev/null -T 10 https://example.com` timed out at TCP/443. [CONFIRMED] Merely selecting hostlist mode did not make the current configuration safe; the runtime command previously showed no populated hostlist argument. [DECISION] Do not run additional connectivity diagnostics. Move directly to Zapret2's documented `autohostlist` mode, which supports automatic hostlist learning for failed connections while retaining hostlist filtering. [STATUS] Zapret2 is currently running and must be stopped before changing MODE_FILTER.

SYNC 2026-09-21: User provided the canonical Master Plan and Master Prompt GitHub links again. No router command was executed in this turn and no OpenWrt state changed. Current factual state remains: Zapret2 is running in MODE_FILTER=hostlist; the identical router-side HTTPS test to example.com timed out in hostlist mode; the prior controlled stop/bypass test proved HTTPS succeeds when Zapret2 is stopped. The next planned action remains the reversible transition from hostlist to autohostlist, beginning by stopping Zapret2. No additional diagnostic tests are authorized/needed before that controlled configuration change.
...
SYNC 2026-09-21: User corrected workflow compliance: every cycle must be ordered as **user output → assistant response → master-plan synchronization → master-prompt synchronization only if a workflow/safety rule changed**. Assistant must not issue the next router command in the same response that reports/records the current user result. [RULE] First record the user's exact factual result; only after plan synchronization may a subsequent assistant turn provide the next command. [CONFIRMED CURRENT RESULT] User reported empty output from the MODE_FILTER hostlist→autohostlist `sed` command; this is consistent with successful `sed` and no error was reported. Zapret2 remains stopped. No additional router command is issued in this turn.

SYNC 2026-09-21: User started Zapret2 after changing MODE_FILTER to autohostlist. [CONFIRMED] `/etc/init.d/zapret2 start` completed without reported error. NFQWS2 started on queue 300; nftables rules were applied for TCP/80,443 and UDP/443. Runtime arguments now show `--hostlist-auto=/opt/zapret2/ipset/zapret-hosts-auto.txt` for TCP/80 and TCP/443 with the configured automatic-learning thresholds, and QUIC uses `--hostlist=/opt/zapret2/ipset/zapret-hosts-auto.txt`. [CONFIRMED] Zapret2/NFQWS2 is running in autohostlist mode. [STATUS] STAGE 11 remains IN_PROGRESS; the earlier causal evidence that broad/active NFQWS2 interception can break HTTPS remains valid. [WORKFLOW] This turn records the current result only; no next router command is issued in the same response.

SYNC 2026-09-21: User said "Продолжай" after the autohostlist start result was recorded. [NEXT ACTION] The next single router command is the already selected functional HTTPS control test against the same endpoint used for causal comparison. No configuration change is made by this test.

SYNC 2026-09-21: Autohostlist HTTPS control test PASS. User executed `wget -O /dev/null -T 10 https://example.com`. Result: DNS resolved to 172.66.147.243, TCP connection to port 443 succeeded, and 559 bytes were downloaded to /dev/null. [CONFIRMED] Router-side HTTPS works with Zapret2/NFQWS2 running in autohostlist mode. This differs from the previous broad `MODE_FILTER=none` and hostlist-mode failures, where the same test timed out. [STATUS] STAGE 11 remains IN_PROGRESS. [IMPORTANT] This is a functional success for the current autohostlist mode, but it does not yet establish client-side Internet success. No configuration or service state was changed by the test. [WORKFLOW] No next router command is issued in this same response.

SYNC 2026-09-21: User said "Продолжай" after the autohostlist router-side HTTPS control test passed. [CONFIRMED] Current state remains Zapret2/NFQWS2 running in MODE_FILTER=autohostlist; router-side HTTPS to example.com succeeds and downloaded 559 bytes. [NEXT] Validate the remaining client-side functional Internet path without changing router configuration. The client should be connected to the MikroTik Wi-Fi and test `https://example.com` and `https://www.google.com`; YouTube success alone is insufficient. No router command is issued in this synchronization turn. [STATUS] STAGE 11 remains IN_PROGRESS.
...
SYNC 2026-09-21: User said "Продолжай" to resume the previously selected Telegram/Zapret functional validation. [CONFIRMED] No router configuration or service state changed from the preceding access-address clarification. Current Zapret2/NFQWS2 state remains RUNNING in MODE_FILTER=autohostlist; router-side HTTPS and client-side control-site HTTPS are confirmed PASS. [NEXT] Begin Telegram validation with one read-only router-side HTTPS test to `https://api.telegram.org`; do not change Zapret2 configuration or restart services before observing this result.


SYNC 2026-09-21: Telegram API control test attempted on OpenWrt: `wget -O /dev/null -T 10 https://api.telegram.org`. [RESULT] `Failed to send request: Operation not permitted`. [CONFIRMED] This is not a timeout or DNS failure; the HTTPS request was rejected locally at send/packet-processing level. [STATUS] STAGE 11 remains DONE for the previously validated general Zapret2 path; Telegram-specific validation is now a separate troubleshooting task. [NEXT] Do not change Zapret2 yet. First perform one read-only check of the active nftables Zapret2 table/rules to determine whether Telegram's traffic is being locally rejected/queued by the current autohostlist path.


SYNC 2026-09-21: User supplied the full live `nft list table inet zapret2` output. [CONFIRMED] Zapret2 has `postnat_hook` and `prenat_hook` hooks; `postnat` queues IPv4 UDP/443 packets 1-5 and TCP/80,443 packets 1-20 to NFQUEUE 300 with `queue flags bypass`; `prenat` queues corresponding WAN replies for UDP/443 packets 1-3 and TCP/80,443 packets 1-10. [CONFIRMED] `wanif` is `phy0-sta0`, `lanif` is `br-lan`, and `forward_hook`, flow-offload chains, `postrouting`, and `prerouting` are empty. [CONFIRMED] No explicit nft `drop`/reject rule for TCP/443 or Telegram is visible in the Zapret2 table; the only explicit drop rules shown are ICMP time-exceeded handling in `prerouting_hook`. [IMPORTANT] The table confirms broad NFQUEUE interception of TCP/443 but does not by itself prove that NFQWS2 is the cause of the Telegram `Operation not permitted` result. [STATUS] STAGE 11 remains DONE; Telegram-specific troubleshooting remains separate. [NEXT] Do not change Zapret2 yet. The current evidence requires a targeted read-only check of the NFQWS2 process/runtime state before any configuration change. No router state changed from this nft inspection.

SYNC 2026-09-21: User said "Продолжай" after the full Zapret2 nftables inspection was synchronized. [CONFIRMED] No router configuration or service state changed after the nft inspection. [CURRENT STATE] Zapret2/NFQWS2 remains running in MODE_FILTER=autohostlist; Telegram API test previously returned `Operation not permitted`; the live nft table shows broad TCP/443 NFQUEUE interception but no explicit TCP/443 drop/reject. [NEXT] Perform one read-only inspection of the current NFQWS2 process status (including PPID/state/resource fields) before considering any configuration change.

SYNC 2026-09-21: User attempted the planned read-only NFQWS2 status inspection: `cat /proc/$(pidof nfqws2)/status | grep -E '^(Name|State|PPid|Uid|VmRSS|Threads):'`. [RESULT] `pidof nfqws2` returned PID 6411, but `/proc/6411/status` no longer existed when `cat` opened it. [CONFIRMED] NFQWS2 PID changed/disappeared during the command, so no process-state conclusion can be made from this attempt. [IMPORTANT] No configuration or service state was intentionally changed by the command. [STATUS] STAGE 11 remains DONE; Telegram-specific troubleshooting remains separate. [NEXT] Do not issue a replacement command in this same synchronization turn; first record this race condition, then select a single read-only process check that safely handles a changing PID.

SYNC 2026-09-21: User said "Продолжай" after the NFQWS2 PID race result was recorded. [CONFIRMED] No router configuration or service state changed after the failed `/proc/6411/status` read. [NEXT] Use a single read-only process listing that captures the currently running `nfqws2` process without relying on a PID that may disappear between `pidof` and `cat`. No Zapret2 configuration change or restart.

SYNC 2026-09-21: User ran the planned read-only process listing `ps w | grep '[n]fqws2'`; result was empty. [CONFIRMED] No `nfqws2` process was visible at that moment. [IMPORTANT] This differs from the earlier state where NFQWS2 was running; the process may have exited or may be supervised/restarted under another process name. No configuration change was made by this command. [STATUS] STAGE 11 remains DONE; Telegram-specific troubleshooting remains separate. [NEXT] Do not change Zapret2 configuration yet; first record this current process absence and then determine service state with one read-only check.

SYNC 2026-09-21: User requested to continue Telegram troubleshooting and supplied forum comments claiming Telegram commonly uses direct IP traffic, hostlists may be ineffective, Telegram IP ranges can be handled via ipsets, and an example nfqws2 MTProto/TTL strategy. [USER-SUPPLIED REFERENCE] These comments are recorded as a hypothesis/reference only; they are not yet validated against the installed Zapret2 v1.0.3 syntax or current runtime. [WORKFLOW] Continue with read-only diagnostics first; do not apply the supplied configuration snippet or TTL changes yet.

SYNC 2026-09-21: User ran `/etc/init.d/zapret2 status`; exact result: `not running`. [CONFIRMED] Zapret2 service is currently stopped. This is consistent with the earlier empty `ps w | grep '[n]fqws2'` result. [IMPORTANT] Do not infer the reason for the stop yet; no restart or configuration change has been performed. [STATUS] STAGE 11 general Zapret functional validation remains DONE; Telegram-specific troubleshooting remains separate and is currently blocked on identifying why the service is stopped.

SYNC 2026-09-21: User asked why Zapret2 is not running, noting that it had been running previously. [CONFIRMED HISTORY] Earlier testing showed Zapret2 was started successfully with the autohostlist configuration and Telegram troubleshooting then observed an intermittent nfqws2 PID race. [CURRENT FACT] The latest `/etc/init.d/zapret2 status` reports `not running`. [LIMITATION] The available evidence does not yet establish whether Zapret2 crashed, was stopped by the system, or exited due to an error. Do not attribute a cause without logs/evidence.

SYNC 2026-09-21: User requested continuation after confirming Zapret2 had previously been running and is now stopped. [NEXT DIAGNOSTIC] The planned next step is a read-only inspection of the Zapret2 service log to determine whether the stop was caused by an error, exit, or other event. No configuration or service state should be changed in this step.

SYNC 2026-09-21: User supplied Zapret2 status/log output. [CONFIRMED ROOT CAUSE] Kernel log shows `oom-kill: ... task=nfqws2,pid=6411` followed by `Out of memory: Killed process 6411 (nfqws2)`. Therefore nfqws2 was terminated by the kernel OOM killer at 07:43:37 on Sep 21 2026. Earlier PID-race observation is explained by this process termination. [HISTORY] Another nfqws2 instance (PID 4894) is visible in the kernel log at Sep 20 21:27:07. [CURRENT] `/etc/init.d/zapret2 status` is `not running`. [STATUS] Telegram troubleshooting is BLOCKED pending recovery of Zapret2; do not change Telegram filters or apply TTL/IPset suggestions yet.

SYNC 2026-09-21: User asked whether OOM could be caused by many files in RAM and whether they can be moved to USB. [TECHNICAL NOTE] Files stored on tmpfs/RAM can consume RAM, but ordinary files on the extroot USB filesystem do not consume their full size as RAM. Moving persistent files to USB can reduce flash/tmpfs pressure, but it does not by itself fix an OOM caused by process RSS. Zapret2's generated runtime state, hostlists, logs, and temporary files must be distinguished from RAM-backed tmpfs before moving anything. [NEXT] First perform a read-only memory/filesystem-location check; no deletion, relocation, restart, or configuration change.

SYNC 2026-09-21: User ran the RAM/tmpfs check. Exact result: /tmp tmpfs 26.8M total, 1.8M used, 25.0M available (7%); /overlay is /dev/sda2 6.6G total, 13.5M used, 6.2G available; root overlayfs uses the same USB-backed extroot. The `du ... | sort -h` portion did not complete because BusyBox `sort` reported `sort: unknown sort type`. [CONCLUSION] /tmp is only 1.8M used, so there is no evidence that a large amount of persistent data in /tmp caused the OOM. Do not move files to USB based on this result. [NEXT] A separate read-only RAM/process check is needed to identify memory pressure; no deletion, relocation, or service restart.

SYNC 2026-09-21: User supplied `top -b -n 1 | head -25`. Current memory: 35296K used, 19556K free, 1756K buff, 1900K cached; load 0.01/0.03/0.00. Top listed users are hostapd (~3764K VSZ), wpa_supplicant (~3752K VSZ), netifd (~2908K), jailed ntpd/dnsmasq/hostapd/wpa_supplicant (~2488K VSZ each), logd (~2072K), procd (~1792K), dnsmasq (~1632K), odhcpd (~1496K). [INTERPRETATION] Current RAM pressure is low: ~19.5 MiB free plus cache/buffers. No large persistent /tmp usage was found. The OOM event therefore appears to have been transient; the current snapshot does not identify a large RAM consumer. Do not move files to USB based on this evidence. [IMPORTANT] CPU is currently mostly kernel/system/softirq, but this one-shot snapshot is not enough to attribute the prior OOM.

SYNC 2026-09-21: User supplied `free -h; swapon -s`. RAM: total 54852K, used 28824K, free 19484K, buff/cache 6544K, available 10464K. Swap: total 550904K, used 4252K, free 546652K. Active swap devices: /dev/sda1 524284K priority -2 and /dev/zram0 26620K priority 100; swapon reports 0 used on sda1 and 4252K used on zram0. [INTERPRETATION] Swap is active and mostly unused; zram has only ~4.2 MiB used. The USB swap is available as fallback. [USER QUESTION] User asks whether there is a historical memory-usage journal after the OOM victim has already been killed. [FACT] The current `free/top` snapshots cannot reconstruct the peak that caused the earlier OOM. Kernel logs retain the OOM event/victim details, but a continuous memory-history graph/log requires a collector that was already running (e.g. collectd/rrdtool or similar). No such collector has been established yet.

SYNC 2026-09-21: User attempted the planned extended OOM log query using `/bin/logread`, but the shell returned `/bin/logread: not found`. [CORRECTION] The earlier command used `logread` successfully, so the utility is available via PATH but not at /bin/logread on this system. No diagnostic data was changed or lost by this failed command. [NEXT] Use the same available `logread` command without the incorrect absolute path.

SYNC 2026-09-21: Extended kernel OOM log provided. [CRITICAL FACT 1] On Sep 20 21:27:07, `https-dns-proxy` (PID 3221) invoked the OOM killer and was killed; immediately before that dnsmasq reported `Maximum number of concurrent DNS queries reached (max: 150)`. The OOM snapshot showed two https-dns-proxy processes with RSS 629 and 557 pages (~2.5 and ~2.2 MiB), nfqws2 PID 4894 with RSS 370 pages (~1.45 MiB), and other normal processes. [CRITICAL FACT 2] On Sep 21 07:43:37, the diagnostic command itself included `grep` (PID 6867), which invoked the OOM killer; the kernel then killed nfqws2 PID 6411. The OOM snapshot showed 16,384 RAM pages, 2,671 reserved, only 1 page in swap cache, and 546,620 kB free swap out of 550,904 kB total. [IMPORTANT] Thus there were at least two separate OOM events. The second occurred while ample swap space remained and while running the diagnostic grep; this does not prove grep caused the underlying memory pressure, only that its allocation triggered OOM selection at that moment. [CORRECTION TO EARLIER INTERPRETATION] The current evidence strongly ties the first OOM to the previously observed https-dns-proxy/DNS-query pressure, and shows the second OOM was triggered during a log/grep command while nfqws2 was resident. Do not restart Zapret2 or change Telegram rules yet. Avoid large `logread | grep` pipelines because the diagnostic itself has now coincided with an OOM event.

SYNC 2026-09-21: User said "Продолжай" after the dual-OOM evidence was recorded. [CONFIRMED] No router state changed since the extended OOM log result. [NEXT] Perform exactly one small read-only memory-counter inspection without a grep pipeline, to avoid repeating the diagnostic pattern that coincided with the second OOM. Use `sed -n '1,35p' /proc/meminfo`. Do not restart Zapret2, change Telegram rules, or move files to USB in this step.


SYNC 2026-09-21: `/proc/meminfo` read-only check PASS. [CONFIRMED] MemTotal 54852 kB; MemFree 18764 kB; MemAvailable 9772 kB; Buffers 1764 kB; Cached 3596 kB; SwapTotal 550904 kB; SwapFree 546652 kB. [CONFIRMED] Anonymous memory is low (AnonPages 1216 kB), while kernel Slab is 8776 kB, of which SUnreclaim is 7532 kB and SReclaimable 1244 kB. Dirty/Writeback are 0. [INTERPRETATION] Current state is not an active severe memory-exhaustion condition: about 18.3 MiB MemFree and 9.5 MiB MemAvailable remain, with about 533.8 MiB swap free. The snapshot does not identify a large user-space file/tmpfs consumer. The relatively large unreclaimable kernel slab is notable and is a better direction for future diagnosis than moving files to USB, but this single snapshot does not establish its cause. [STATUS] Telegram-specific troubleshooting remains BLOCKED pending stable Zapret2 recovery; STAGE 11 general validation remains DONE. [NEXT] Do not restart Zapret2 or alter Telegram rules yet. No file relocation is justified by this result.
