...
## SYNC RULE UPDATE 2026-09-19 — STAGE 11 activation gate
- Treat `INIT_APPLY_FW=1` in Zapret2 `config.default` as a safety-relevant default: service activation may apply firewall integration.
...
## EXECUTION MODE — смелый, но управляемый workflow — 2026-09-25

Проект по умолчанию работает в режиме **EXECUTION**, а не в режиме длинного повторного объяснения.

### 1. Команда пользователя «ПРОДОЛЖАЙ»
Фразы пользователя:
- «продолжай»;
- «продолжай по плану»;
- «идём дальше»;
...
- не просит повторного подтверждения;
- не пересказывает весь мастер-план;
- не перечисляет уже закрытые проверки;
- не предлагает несколько равноправных вариантов вместо следующего шага;
- выполняет **ровно один** следующий router command/test.

### 2. Когда AI должен действовать смелее
После обязательного preflight AI должен предпочитать действие, а не дополнительное рассуждение, когда одновременно выполнены условия:
- следующий шаг явно определён MASTER PLAN;
- capability audit не выявил дубликата/конфликта;
- операция read-only, ограниченная, обратимая или уже предусмотрена утверждённым этапом;
- не требуется отдельное подтверждение безопасности.

В таком случае дополнительная серия «а можем ли мы ещё проверить...» не нужна.

### 3. Когда всё ещё обязательно остановиться и спросить
Отдельное подтверждение пользователя требуется перед:
- flash/sysupgrade;
- форматированием, переразметкой или удалением данных;
- массовым upgrade/replace пакетов;
- изменением, которое может лишить пользователя доступа к роутеру;
- созданием/включением нового default route или полной VPN-маршрутизации;
- заменой рабочей конфигурации на рискованный эксперимент;
- действиями с секретами/ключами, требующими передачи секрета в чат;
- иными явно destructive или труднообратимыми действиями.

Сам факт того, что команда изменяет UCI, **не означает автоматически**, что требуется отдельное подтверждение: изолированные, обратимые и неактивируемые конфигурационные заготовки можно делать по плану без повторного согласования.

### 4. Формат ответа в EXECUTION режиме
Перед командой достаточно:
**STATUS → что уже есть → что именно меняем сейчас → команда.**

После результата:
**RESULT → классификация → изменение статуса при необходимости → синхронизация MASTER PLAN → следующий один шаг.**

Не повторять историю и теорию, если они не влияют на текущий шаг.

### 5. Принцип «не тестировать ради теста»
Если предыдущие A/B-проверки уже закрыли гипотезу, AI не должен автоматически генерировать ещё десятки близких тестов. Нужен новый вопрос/гипотеза или следующий gate MASTER PLAN.

### 6. Исправление локальной ошибки не должно останавливать workflow
Если команда не выполнилась из-за синтаксиса BusyBox, неправильного ключа или особенностей конкретной утилиты, это классифицируется как TOOL/SYNTAX BLOCKED. После исправления команды AI продолжает тот же этап, а не перестраивает весь план с нуля.

### 7. Не повторять preflight в ответе
Preflight выполняется AI внутренне. Пользователю не нужно каждый раз читать длинное описание того, что документы были перечитаны. Достаточно показать только вывод, влияющий на следующий шаг.

### 8. Стандартная пользовательская формула
Для продолжения проекта пользователь может писать просто:
**«Продолжай.»**

Для большей жёсткости:
**«Продолжай строго по MASTER PLAN с текущей точки. Один шаг.»**

Для возврата после паузы:
